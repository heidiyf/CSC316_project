...
Promise.all([
  ...
  data.forEach(d => {
    d.Medal = d.Medal == null ? 'No medal' : d.Medal;
  });
  renderStandaloneScale(data);
}).catch(err => {
  console.error(err);
  const summary = document.getElementById('raceModeSummary');
  if (summary) summary.textContent = `Failed to load data: ${err}`;
});

function renderStandaloneScale(data) {
