 ...
 * CSC316 Final Project  ·  Fully self-contained module
 *
 * ┌──────────────────────────────────────────────────────────────┐
 * │  Drop these three files in the same folder as index.html:    │
 * │    • sportpark.js            ← this file                     │
 * │    • sports-park.png         ← isometric park image          │
 * │    • olympics_dataset.csv    ← medal data                    │
 * │                                                              │
 * │  index.html only needs:                                      │
 * │    <script src="https://cdnjs.cloudflare.com/ajax/libs/      │
 * │      d3/7.9.0/d3.min.js"></script>                           │
 * │    <script src="sportpark.js"></script>                       │
 * │                                                              │
 * │  No other JS, no other CSS, no function calls required.      │
 * └──────────────────────────────────────────────────────────────┘
 *
 * This file self-boots on DOMContentLoaded and:
 *   1. Injects its own <style> into <head>
 *   2. Builds the full page into <body>
 *   3. Loads olympics_dataset.csv with d3.csv()
 *   4. Renders the interactive park map + side panel
 */
...
(function () {
  ...
  /* ── Venue definitions  (x,y = dot position in 1536×1024 space) */
  const VENUES = [
    {
      id: 'athletics', label: 'Athletics Stadium',
      venue: 'Track & Field Oval',
    ...
    {
      id: 'archery', label: 'Precision Range',
      venue: 'Archery & Shooting Range',
    ...
    {
      id: 'indoor', label: 'Multi-Sport Hall',
      venue: 'Indoor Arena',
      color: '#1e3a8a', accent: '#60a5fa',
      x: 1090, y: 240,
      desc: 'The dome hosts racket sports, combat and heavy lifting',
      sports: ['Badminton', 'Table Tennis', 'Weightlifting', 'Fencing', 'Gymnastics'],
    },
    {
      id: 'aquatics', label: 'Aquatics Centre',
      venue: 'Swimming Pool',
      color: '#0369a1', accent: '#38bdf8',
      x: 258, y: 456,
      desc: 'Where USA and China clash hardest in recent Games',
      sports: [
        'Swimming', 'Diving', 'Water Polo',
        'Artistic Swimming', 'Synchronized Swimming',
        'Marathon Swimming', 'Marathon Swimming, Swimming',
      ],
    },
    {
      id: 'combat', label: 'Combat Arena Pavilion',
      venue: 'Boxing Ring & Combat Hall',
      color: '#5b21b6', accent: '#c084fc',
      x: 645, y: 455,
    ...
    {
      id: 'extreme', label: 'Skate & Climb Park',
      venue: 'Extreme & Other Sports',
      color: '#374151', accent: '#9ca3af',
      x: 570, y: 580,
      ...
      sports: [
        ...
        'Croquet', 'Jeu De Paume', 'Motorboating',
        'Racquets', 'Roque', 'Figure Skating', 'Ice Hockey', 'Polo',
      ],
    },
    {
      id: 'ballsports', label: 'Multi-Court Zone',
      venue: 'Ball Sports Complex',
      color: '#065f46', accent: '#34d399',
      x: 1020, y: 490,
      desc: 'Every ball sport from football to golf',
      sports: [
        'Football', 'Basketball', '3x3 Basketball', '3x3 Basketball, Basketball',
    ...
    {
      id: 'gymnastics', label: 'Gymnastics Hall',
      venue: 'Gymnastics Floor',
      color: '#9d174d', accent: '#f472b6',
      x: 295, y: 655,
    ...
    {
      id: 'equestrian', label: 'Equestrian Arena',
      venue: 'Sand Arena',
      color: '#78350f', accent: '#fcd34d',
      x: 910, y: 600,
      desc: 'Horse and rider in perfect harmony on the sand',
      sports: ['Equestrian', 'Equestrianism'],
    },
    {
      id: 'cycling', label: 'Cycling Circuit',
      venue: 'Road & Track Cycling',
      color: '#14532d', accent: '#4ade80',
      x: 790, y: 840,
      desc: 'Speed on every surface — road, track, mountain and dirt',
      sports: [
        'Cycling', 'Cycling Road', 'Cycling Track',
        'Cycling BMX Racing', 'Cycling BMX Freestyle', 'Cycling Mountain Bike',
        'Cycling Road, Cycling Mountain Bike',
        'Cycling Road, Cycling Track', 'Cycling Road, Triathlon',
      ],
    },
    {
      id: 'watersports', label: 'Water Sports Lake',
      venue: 'Open Water & Lake',
      color: '#164e63', accent: '#22d3ee',
      x: 112, y: 760,
...
#sp-loading::before {
  ...
  border-radius: 50%;
  animation: sp-spin .8s linear infinite; flex-shrink: 0;
}
...
.sp-pill-dot { width: 7px; height: 7px; border-radius: 50%; flex-shrink: 0; }
...
.sp-hist-pill:hover { background: rgba(0,0,0,.09); color: #555; }
  ...
  function buildSkeleton() {
    ...
    header.innerHTML = `
      ...
      <p>Click any glowing venue dot to explore its sports.
         Select a sport pill to reveal all-time medal leaders
         and an official link to learn more.</p>
      <div id="sp-sport-count">Loading…</div>
    `;
    document.body.appendChild(header);

    const loading = document.createElement('div');
    loading.id = 'sp-loading';
    loading.textContent = 'Loading medal data…';
    ...
    return { loading, outer };
  }

  /* ═══════════════════════════════════════════════════════════════
     COMPUTE MEDAL DATA FROM CSV ROWS
  ═══════════════════════════════════════════════════════════════ */
  function computeMedals(rows) {
    // sportMedals[sport][noc] = { Gold, Silver, Bronze }
    const sportMedals   = {};
    const sportRowCount = {};
  ...
  /* ═══════════════════════════════════════════════════════════════
     BUILD MAP + PANEL
  ═══════════════════════════════════════════════════════════════ */
  function buildMap(outer, sportMedals, sportRowCount) {

    /* helper: find top NOC for one medal type in one sport */
    ...
    /* helper: build the expanded sport detail card HTML */
    function sportCardHtml(sport, venueColor) {
      ...
      const row = (emoji, type, leader, cls) => leader ? `
        <div class="sp-medal-row">
          ...
          <div class="sp-m-n ${cls}">${fmt(leader.n)}</div>
        </div>` : '';
      ...
      return `<div class="sp-card">
        ...
        <div class="sp-card-note">${fmt(total)} medal records in dataset</div>
      </div>`;
    }

    /* enrich venue objects */
    VENUES.forEach(v => {
    ...
    img.src = IMG_PATH; img.alt = 'Olympic Sports Park';
    box.appendChild(img);
    ...
    // core
    dots.append('circle').attr('class','sp-dot-core').attr('r',11)
      .attr('fill', d=>d.color).attr('stroke','#fff').attr('stroke-width',2.5)
      .attr('filter','url(#sp-glow)');
    // centre
    dots.append('circle').attr('r',3.5).attr('fill','#fff').attr('opacity',.95).attr('pointer-events','none');
    // label
    dots.append('text').attr('class','sp-dot-label')
      .attr('x',0).attr('y',-20).attr('text-anchor','middle')
      .attr('fill', d=>d.accent).text(d=>d.venue);
    ...
    function openPanel(venue) {
      ...
      const histHtml = venue._unknown.length ? `
        <div>
          ...
          <div class="sp-hist-pills">
            ${venue._unknown.map(s =>
              `<a class="sp-hist-pill" href="${getSportUrl(s)}" target="_blank" rel="noopener noreferrer">${s} ↗</a>`
            ).join('')}
          </div>
        </div>` : '';
      ...
      bodyEl.innerHTML = `
        <div class="sp-stats">
          <div class="sp-stat"><div class="sp-stat-n">${fmt(venue.totalMedals)}</div><div class="sp-stat-l">Medals</div></div>
        ...
        <div>
          ...
          <div class="sp-pills" id="sp-pills">${pillsHtml}</div>
          ${histHtml}
        </div>
    ...
    dots.on('click', (event, d) => {
      ...
      if (activeVenueId === d.id) closePanel();
      else openPanel(d);
    });
  ...
  /* ═══════════════════════════════════════════════════════════════
     BOOTSTRAP — runs automatically on DOMContentLoaded
  ═══════════════════════════════════════════════════════════════ */
  function bootstrap() {
    injectStyles();
    const { loading, outer } = buildSkeleton();
    ...
    d3.csv(CSV_PATH, d3.autoType)
      ...
      .catch(err => {
        loading.innerHTML = `⚠ <strong>Could not load ${CSV_PATH}</strong> — make sure it is in the same folder as index.html`;
